...
function minikube-install() {
  ...
  echo -e '\e[38;5;198m'"++++ Minicube delete"
  sudo minikube delete
  yes | sudo docker system prune -a
  yes | sudo docker system prune --volumes
  sudo service containerd stop
  for i in $(ps uax | grep -e kube -e containerd | tr -s ' ' | cut -d ' ' -f2); do sudo kill -9 $i; done
  echo -e '\e[38;5;198m'"++++ Clean up Docker Volumes"
  ...
  sudo apt-get install --assume-yes conntrack
  ...
  sudo helm repo update
  sudo helm install buildkite/agent --name bk-agent --namespace buildkite --set agent.token=""
